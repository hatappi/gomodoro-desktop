...
const api = {
  ...
  checkGraphQLConnection: () =>
    invokeIpc<{ isConnected: boolean }>(IPC_CHANNELS.CHECK_GRAPHQL_CONNECTION),

  // Pomodoro management
  getCurrentPomodoro: () =>
    invokeIpc<Pomodoro | null>(IPC_CHANNELS.GET_CURRENT_POMODORO),
  startPomodoro: (input: StartPomodoroParams) =>
    invokeIpc<Pomodoro>(IPC_CHANNELS.START_POMODORO, input),
  ...
  createTask: (input: { title: string }) =>
    invokeIpc<Task>(IPC_CHANNELS.CREATE_TASK, input),
  updateTask: (input: { taskId: string; title: string }) =>
    invokeIpc<Task>(IPC_CHANNELS.UPDATE_TASK, input),
  deleteTask: (input: { taskId: string }) =>
    invokeIpc<boolean>(IPC_CHANNELS.DELETE_TASK, input),
};
